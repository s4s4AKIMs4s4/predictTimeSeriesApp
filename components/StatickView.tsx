...
import useMetricks, { IStatickDomainData } from "../features/metrics.tsx/useMetricks"
...
const StatickView: React.FC<ISubPage> = ({ ticker, comparedTicker, timePeriodO }) => {
    ...
    const { getMSE, getStaticsValue, max, getLastData, getModStatistic,generateLastTime } = useMetricks()
    const { getDomainRadarData } = useStatickChart()
    const [domainFunction, setDomainFunciton] = useState<domainFunctionEnum>(domainFunctionEnum.MSE)
    const [lastData, setLastData] = useState<ILastDate>()
    const [lastDataFunciton, setLastDataFunciton] = useState<lastDataEnum>(lastDataEnum.CLOSE)
    const [radarData, setRadarData] = useState<Array<{ [key: string]: string | number }>>()
    const [
        { dataTicker, isLoadingDataTicker, isErrorDataTicker },
        { dataComparedTicker, isLoadingComparedTicker, isErrorCompared }] = useGetMarcetplacesData(ticker, comparedTicker)

    const getStaticsFunction = () => {
        if (domainFunction === domainFunctionEnum.MSE) return getStaticsValue
        else if (domainFunction === domainFunctionEnum.MODA) return getModStatistic
        else return null
    }

    const getStatistickCompareObject = (KdataTicker: klinecharts.KLineData[], KdataCompareTicker: klinecharts.KLineData[]) => {
            ...
            isErrorCompared || isErrorDataTicker ? <>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', flexDirection: 'column' }}>
                    <div style={{ width: '100%', height: '500px' }}>
                        ...
                        <Heading as='h2' className={StatickChartStyles.radarСharsView__item} textAlign={'center'} size='xl' noOfLines={3}>
                            Domain Chart
                        </Heading>


                        <Select className={StatickChartStyles.radarСharsView__item} onChange={changeSelectDomain} defaultValue={domainFunctionEnum.MODA} size='md'>
                            <option value={domainFunctionEnum.MODA}>moda</option>
                            <option value={domainFunctionEnum.MSE}>mse</option>
                        </Select>

                        {
                            radarData && max && <RadarChartView
                    ...
                    <Heading className={StatickChartStyles.synchronizationView__header} as='h2' textAlign={'center'} size='xl' noOfLines={3}>
                        Synchronization charts
                    </Heading>

                    <Select className={StatickChartStyles.synchronizationView__item} onChange={changeSelectLast} defaultValue={lastDataEnum.CLOSE} size='md'>
                    ...
                    {
                        lastData && max && <SynchronizationChart ticker= {ticker} comparedTicker = {comparedTicker}  lastData={lastData} lastDataFunciton={lastDataFunciton} />
                    }
                </div>
            </>
        }
