...
const SearchView: React.FC = () => {
    const { ticker, setTicker, setCompatedTicker, comparedTicker } = useContext(TickerContext)
    ...
    return <div className={SearchViewCss.searchViewContainer}>
        ...
        <TickerInput setTicker={setTicker} placeholder={'Enter Ticker or company name'} />
            ...
            isCompareMode && <>
                ...
                <form onSubmit = {(e:React.FormEvent) => {
                    ...
                    <RadioGroup name="time" defaultValue={String(TimePeriod.THREE_DAYS)}>
                        <HStack spacing='24px'>
                            <Radio colorScheme='green' value = {String(TimePeriod.THREE_DAYS)}>Three Days</Radio>
                            <Radio colorScheme='green' value = {String(TimePeriod.WEEK)}>Week</Radio>
                        </HStack>
                    </RadioGroup>
        ...
        <div className={SearchViewCss.searchViewContainer__linkWrapper}>
            <NextLink legacyBehavior passHref href={{ pathname: '/predict', query: { ticker: `${ticker}` } }}>
                <Link className={isCompareMode && SearchViewCss.link__disabled} color='teal.500'>
                    predict
                </Link>
            </NextLink>
            <NextLink legacyBehavior passHref href={{ pathname: '/predict', query: { ticker: `${ticker}` } }}>
            ...
            <span className={SearchViewCss.fakeLink} onClick={compareClickHandler} >
                {
