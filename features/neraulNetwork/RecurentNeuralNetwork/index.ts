...
export const getMaxThreshold = (data: Array<any>) => {
    ...
    const maxStockObject = data.reduce(
        (acc, currentValue, index) => {
            const currentPrice = currentValue.close;
            const isMaxInTheEnd = data.length - index < 5 && currentPrice > acc;
            if (currentPrice > acc.maxPrice)
                return { maxPrice: currentPrice, isMaxInTheEnd };
            else return { ...acc };
        },
    ...
    if (maxStockObject.isMaxInTheEnd)
        return (
            maxStockObject.maxPrice +
            maxStockObject.maxPrice * coficentThreshold
        );
...
export const makePredictions = (X, model, maxThreshold) => {
    const predictedResults = model.predict(
        tf.tensor2d(X, [X.length, X[0].length]).div(tf.scalar(maxThreshold))
    );
