...
const useMetricks = () => {
    ...
    const generateLastTime = (data: klinecharts.KLineData[]) => {
        return data.map((value) => {
            return {
                ...value,
                timestamp: new Intl.DateTimeFormat("en-US").format(
                    value.timestamp
                )
            };
        });
    ...
    const getMSE = (
        data: klinecharts.KLineData[],
        numberOfDays: number,
        key: keyof klinecharts.KLineData
    ) => {
    ...
    const getMode = (
        data: klinecharts.KLineData[],
        atribute: keyof klinecharts.KLineData
    ) => {
        const modeAr = data
